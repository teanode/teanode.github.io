...
**`externalAddresses`**——ExternalAddresses 是邮件实际到达这台服务器的地址，当它们不是服务器为自己发现的那个地址时：前面的一个中继、来自别处的一条隧道、一个负载均衡器。DNS 建议会拿每个 MX 主机的 A 记录去比对服务器从外面看到的地址，因为对一台直接面向互联网的服务器来说，那是运维者唯一查不到的值；而通过别的东西到达的服务器会在一条本来正确的记录上永远通不过这项检查。把那些地址列在这里，它们就算作正确。名字和地址都接受，名字会在计算建议时解析，所以一个地址会变的转发器也一直是对的。

**`trustedProxies`**——TrustedProxies 是这台服务器被经由哪些地址访问，当它位于 CDN、负载均衡器或反向代理之后时：一个地址，或者 CIDR 形式的一段范围。经过代理的连接来自代理，所以没有这一项时，每一条审计记录和每一个「上次使用来自」记下的都是代理的地址，而不是人的。客户端地址在 `X-Forwarded-For` 里，那是代理加上去的——而任何能直接连到这台服务器的人也能伪造它，所以只有当连接来自这里列出的地址之一时才会去读它。留空是安全的默认值，对一台直接面向互联网的服务器来说也是对的：那时连接自己的地址就是真相。
...
告诉邮件程序读取邮件时该连到哪里，也就是邮箱设置里"邮件程序"页面显示的内容。

通常它跟随服务器——`server.name`，以及 `listen.imap` 和 `listen.imaps` 里的端口——这三项都可以留空。当前面有东西转发了不同的端口时才设置它们：一个发布 10993 的容器，或者一个在外面接管 993 的路由器。不设的话，页面会把进程碰巧绑定的端口交给别人，而那不是他们的邮件程序能连上的端口。它是读取版的 `smtp.submission`，和它一样不改变服务器做什么，只改变它说什么。

**`host`**——连接到什么。空表示 `server.name`。

**`port`**——先明文再转成 TLS 的那种连接用的端口。空表示 `listen.imap` 里的端口。

**`tlsPort`**——从第一个字节起就是 TLS 的那种连接用的端口，也是大多数邮件程序会尝试的。空表示 `listen.imaps` 里的端口。
...
**`minimumMessages`**——在允许分类器贡献任何分数之前，垃圾邮件和非垃圾邮件**每一类**各自必须先学过多少封。用四封邮件训练出来的分类器只会自信地出错；而用两百封一类、四十封另一类训练出来的，会对所有不像那四十封的东西自信地出错。在两个计数都达到之前，分类器保持沉默，仪表盘的垃圾邮件明细里也不会出现 `BAYES` 这一行。
...
**`maxRoundsPerReply`**——一次起草的运行可以走多少轮。现在一封回信是一次对话循环的运行，
在动笔之前要读这个会话、邮箱、通讯录和日程：「你周四有空吗」光看这封邮件是答不上来的。
默认六轮。

**`maxRoundsPerTriage`**——一次分类的运行可以走多少轮，默认三轮。三者里最小的那个，而且是
有意的：每一封到达的邮件都要分类，而几乎每一封都能从模型眼前的东西里分出来，所以提示词让它
立刻作答，这个数只是那些例外的上限。
...
**`proxyListen`**——受守卫的代理绑定在哪里。这台服务器打开的每一个页面都经由那个代理出去，
而正是它替浏览器解析名字，好让别的东西不去解析：于是检查跑在连接实际建立到的那个地址上，
而不是跑在一个过一会儿可能解析成别的东西的名字上。留空会在 Chrome 够到这台服务器所用的那
一个地址上绑一个没人用的端口——它自己的连接说出那是哪一个，所以不管 Chrome 跑在这台机器上
还是跑在旁边的一个容器里都行，而且不会把这个代理放到这台机器所在的每一个网络上。这个猜测
不对的时候，或者想为防火墙固定端口的时候，就设置它。这个代理需要一个只有这台服务器和它的
Chrome 知道的密码。
