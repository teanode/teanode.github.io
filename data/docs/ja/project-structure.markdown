...
**`internal/configdb`** — PostgreSQL に裏付けられた `config.Store`。設定をテーブルに写し、また戻し、バージョン行で並行書き込みを解決し、それをポーリングして、別のインスタンスが加えた変更に気づきます。`config` と `db` の両方をインポートし、`db` が `config` をインポートするため、独立したパッケージです。
...
**`internal/db`** — GORM を通した PostgreSQL。際限なく増えるデータだけを持ちます。メール、配送、DMARC レポート、使用量のカウンター、テンプレート、レイアウト。エンティティごとに一ファイルで、`internal/models` の共有構造体とは別に GORM モデルを定義するので、API を変えずに保存の形を変えられます。
...
`internal/cmd/server` はすべてに依存します。`configdb` は `config` と `db` に依存し、`internal/cmd` 以外は `configdb` に依存しません。設定をどこに保存するかという選択を、設定を読むすべてのものから遠ざけているのはこれです。`api`、`mx`、`dns`、`mailer` は `config`、`db`、`models`、`util` に依存します。`util` のパッケージは互いと標準ライブラリにしか依存しません。`util` の中のものが `config`、`db`、`models` をインポートしてはいけません。これらは独立したライブラリとして持ち出せるように作られていて、そのいくつかはこのプロジェクトを公開する価値の理由です。
